...
function setMeetingDate(date) {
    const formattedMeetingDate = date.toLocaleDateString("en-GB", {
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
    })
    title.innerHTML = "RDV le " + formattedMeetingDate + " à " + (date.getHours() < 10 ? "0" + date.getHours() : date.getHours()) + 'h' + (date.getMinutes() < 10 ? "0" + date.getMinutes() : date.getMinutes());
}

function setGoDate(time) {
    goDate = new Date(meetingDate);
    goDate.setMinutes(goDate.getMinutes() - time);
    const formattedGoDate = goDate.toLocaleDateString("en-GB", {
        year: "numeric",
        month: "2-digit",
        day: "2-digit",
    })
    subtitle.innerHTML = "Tu dois partir le " + formattedGoDate + " à " + (goDate.getHours() < 10 ? "0" + goDate.getHours() : goDate.getHours()) + 'h' + (goDate.getMinutes() < 10 ? "0" + goDate.getMinutes() : goDate.getMinutes()) + " (trajet de " + Math.ceil(time) + "min)";
    return goDate;
}
...
eiffelMarker.on('click', function(e) {
    userPlace = undefined;
    map.removeLayer(userPath);
    userPath = L.polyline([userPos, eiffelPos], {color: 'green'}).addTo(map);
    userTime = ((map.distance(userPos, eiffelPos) / 1000) / 20) * 60;
